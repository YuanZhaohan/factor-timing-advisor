...
2. 挑战者 `entry_frequency_inverse_sqrt_weekly_binary`：按历史开仓频率降权的周频策略。
...
每个标签先展示加权方式、调仓频率、触发条件和下一交易日执行规则，再依次展示当前信号状态、价格与开平仓点、复合分数与仓位、策略/基准净值、超额净值和历史绩效。页面不使用“主策略/挑战者”称谓。

## 当前复现快照

基于截至 2026-07-09 的 selected-rule 输出，并扣除单边 5bp 换仓成本：

| 策略 | 年化收益 | Sharpe | 最大回撤 |
|---|---:|---:|---:|
| 两速类别等权主策略 | 18.07% | 1.40 | -11.43% |
| 频率平方根倒数挑战者 | 17.53% | 1.35 | -15.39% |
